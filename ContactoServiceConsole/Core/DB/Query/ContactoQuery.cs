...
namespace ContactoServiceConsole.Core.DB.Query
{
    class ContactoQuery
    {
        private IMongoClient _client;
        private IMongoDatabase _database;
        private IMongoCollection<Core.DB.Models.ContactoDBModel> _usersCollection;
        ...
        public ContactoQuery(string connectionString) //COSNTRUCTOR 
        {
            ...
            _usersCollection = _database.GetCollection<Core.DB.Models.ContactoDBModel>("contacto");
        }

        #region CREATE

        public async Task InsertUser(Core.DB.Models.ContactoDBModel user) //CREATE
        {
        ...
        public List<Core.DB.Models.ContactoDBModel> GetAllUsers()
        {
            return _usersCollection.Find(new BsonDocument()).ToList();
        }

        public List<Core.DB.Models.ContactoDBModel> GetUsersByField(string fieldName, string fieldValue)
        {
            var filter = Builders<Core.DB.Models.ContactoDBModel>.Filter.Eq(fieldName, fieldValue);
            var result = _usersCollection.Find(filter).ToList();

            return result;
        }

        public Core.DB.Models.ContactoDBModel GetUsersById(string id)
        {
        ...
        public List<Core.DB.Models.ContactoDBModel> GetUsers(int startingFrom, int count)
        {
